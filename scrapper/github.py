...
class Github():
    ...
    def fetch_following(self, user, depth = 2):
        ...
        if self.options['offline'] == 0:
            ...
            sql="SELECT user2, user1 FROM following WHERE is_deleted=0"
            result = db.read(sql, self.cursor)
            if result:
                for i in result:
                    # user1 = i[0]
    ...
    def fetch_repo_fork(self, user, depth = 2):
        ...
        if self.options['offline'] == 0:
            while count <= depth:
                ...
                for user in self.fork_list[count]:
                    if user not in self.fork:
                        print user
                        # https://api.github.com/users/shagunsodhani/fork?page=1&per_page=100
            ...
            sql="SELECT user1, user2 FROM fork WHERE is_deleted=0"
            # user1's repo forked by user2
            result = db.read(sql, self.cursor)
            if result:
                for i in result:
                    # user1 = i[0]
                    # user2 = i[1]
                    # self.fork[user1].append(user2)
                    self.fork[i[0]].append(i[1])
            while count<=depth:
                temp_fork_list = []
                for user in self.fork_list[count]:
                    for i in self.fork[user]:
                ...
                count+=1
            for i in self.fork:
                self.fork_count[i] = len(self.fork[i])
